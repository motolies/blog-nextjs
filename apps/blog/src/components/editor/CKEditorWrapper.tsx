...
interface CreateEditorConfigParams {
  ...
  onChangeDataRef: React.MutableRefObject<
    (data: string, options?: { shouldSave?: boolean; trigger?: string }) => void
  >;
}
...
function createEditorConfig({
  ...
  const markdownSourceEditingPlugin = (editor: any) => {
    ...
    const markdownIcon =
      '<svg viewBox="0 0 208 128" xmlns="http://www.w3.org/2000/svg"><path d="M15 10h178a5 5 0 0 1 5 5v98a5 5 0 0 1-5 5H15a5 5 0 0 1-5-5V15a5 5 0 0 1 5-5zm0-10C6.7 0 0 6.7 0 15v98c0 8.3 6.7 15 15 15h178c8.3 0 15-6.7 15-15V15c0-8.3-6.7-15-15-15H15z"/><path d="M30 98V30h20l20 25 20-25h20v68H90V59L70 84 50 59v39zm125 0-30-33h20V30h20v35h20z"/></svg>';
  ...
  const fileUploadPlugin = (editor: any) => {
    editor.editing.view.document.on(
      ...
      async (event: any, data: any) => {
        if (data.dataTransfer.files && !data.dataTransfer.files[0].type.includes('image')) {
          event.stop();
          data.preventDefault();
  ...
  return {
    ...
    plugins: [
      ...
      htmlSourceEditingButtonPlugin,
      markdownSourceEditingPlugin,
    ],
      ...
      <CKEditor
        ...
        onError={(error: Error, details?: { phase?: string }) => {
          ...
          if (details?.phase === 'initialization') {
            setInitError(
              '지원되지 않는 CKEditor 설정 충돌이 발생했습니다. 페이지를 새로고침한 뒤 다시 시도해주세요.',
            );
          }
        }}
        onAfterDestroy={() => {
