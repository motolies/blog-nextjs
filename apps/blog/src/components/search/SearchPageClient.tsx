...
/**
 * 검색 결과 화면(클라이언트) — useSearchParams 를 쓰는 유일한 곳이라 (public)/search/page.tsx 의 Suspense 경계 안에서만 렌더된다.
 * q(base64 SearchObject) 가 있으면 그것을, 없고 query(평문 키워드)가 있으면 단일 키워드 검색으로 해석한다.
 */
export default function SearchPageClient() {
  const router = useRouter();
  const searchParams = useSearchParams();
  // get() 은 이미 URL 디코드된 값 — buildSearchHref 가 encodeURIComponent 한 base64 가 원형으로 돌아온다
  const encodedQuery = searchParams.get('q');
  const plainQuery = searchParams.get('query');
  // lg 미만에서 필터 패널 접기 — 기본 접힘이라 결과가 요약 바로 아래 노출된다
  const [filtersOpen, setFiltersOpen] = useState<boolean>(false);

  const queryParam = useMemo(() => {
    if (encodedQuery) {
      const decodeString = base64Decode(encodedQuery);
    ...
    if (plainQuery) {
      const newObj = {
        searchCondition: {
          keywords: [{ id: getTsid().toString(), name: plainQuery }],
          logic: 'AND',
        },
  ...
  const goPage = (newPage: number) => {
    ...
    if (page !== targetPage) {
      ...
      router.push(buildSearchHref(newSearchAllParam));
    }
  };
