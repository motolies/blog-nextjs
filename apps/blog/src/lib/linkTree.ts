...
/**
 * 형제를 fromIndex → toIndex 로 옮기고 sort 를 1..n 으로 **정규화**한 뒤,
 * 값이 실제로 바뀐 노드만 돌려준다(그것만 PUT 하면 된다).
 *
 * 단순히 두 노드의 sort 를 맞바꾸지 않는 이유: 시드나 수기 편집으로 sort 가 중복되거나
 * 구멍이 나 있으면 swap 은 조용히 무동작이 된다(값이 동률이면 백엔드가 code ASC 로 밀어버린다).
 * 전체 정규화는 그런 상태를 지나가면서 함께 고친다.
 *
 * ⚠️ 백엔드에 reorder 엔드포인트는 없다(`PUT /nodes/{id}/reorder` 는 404). 반환값을
 *    `updateNode(id, { sort })` 로 보내는 것이 정석 경로다.
 */
export function renumberSiblings<T extends { id: string; sort: number }>(
  siblings: readonly T[],
  fromIndex: number,
  toIndex: number,
  ...
  return reordered
    .map((node, index) => ({ id: node.id, sort: index + 1, changed: node.sort !== index + 1 }))
