...
export default function SprintPage() {
  ...
  const [sprintDetailData, setSprintDetailData] = useState<any[]>([]);
  const [detailLoading, setDetailLoading] = useState(false);

  const [selectedIssue, setSelectedIssue] = useState<string | null>(null);
  const [issueWorklogData, setIssueWorklogData] = useState<any[]>([]);
  ...
  const loadSprintData = useCallback(async (year: number) => {
    try {
      setLoading(true);
  ...
  const loadSprintDetail = useCallback(async (sprintName: string, worker: string | null = null) => {
    try {
  ...
  const loadIssueWorklog = useCallback(async (issueKey: string) => {
    try {
  ...
  const handleSprintClick = (sprintName: string) => {
    loadSprintDetail(sprintName);
    setSelectedIssue(null);
    setIssueWorklogData([]);
  };

  const handleWorkerClick = (worker: string, sprintName: string) => {
  ...
  const getChartOptions = () => {
    ...
    const series = assigneeArray.map((assignee: any, index: number) => ({
      name: assignee.assignee || '',
      type: 'bar',
      stack: 'total',
      data: sprintArray.map((sprint: string) => assignee.sprintStoryPoints?.[sprint] || 0),
  ...
  const getTableData = () => {
    if (!sprintData)
      return { headers: [] as string[], rows: [] as any[][], sprints: [] as string[] };
    const { sprints = [], assigneeSummaries = [] } = sprintData;
    const sprintArray: string[] = Array.isArray(sprints) ? sprints : [];
    const headers = ['작업자', ...sprintArray, '계'];
    const rows = assigneeSummaries.map((assignee: any) => [
      assignee.assignee,
      ...sprintArray.map((sprint: string) => assignee.sprintStoryPoints?.[sprint] || 0),
      assignee.totalStoryPoints || 0,
    ]);
    const sprintTotalsRow = [
      '스프린트 합계',
      ...sprintArray.map((sprint: string) => {
        return assigneeSummaries.reduce((total: number, assignee: any) => {
          return total + (assignee.sprintStoryPoints?.[sprint] || 0);
        }, 0);
      }),
      assigneeSummaries.reduce(
        (total: number, assignee: any) => total + (assignee.totalStoryPoints || 0),
        0,
      ),
    ];
  ...
  const sprintDetailColumns = useMemo<DataTableColumn[]>(
    () => [
  ...
  const issueWorklogColumns = useMemo<DataTableColumn[]>(
    () => [
  ...
  const sprintDetailTotals = useMemo(() => {
    return sprintDetailData.reduce(
      (totals, item) => {
  ...
  const issueWorklogTotals = useMemo(() => {
    return issueWorklogData.reduce(
      (totals, item) => {
                ...
                {tableData.rows.map((row, rowIndex) => {
                  ...
                  return (
                    <tr
                      ...
                      className={
                        isTotalRow
                          ? 'bg-slate-50/90 font-bold text-[color:var(--admin-text)]'
                          : 'text-[color:var(--admin-text-secondary)]'
                      }
                    >
                      {row.map((cell: any, cellIndex: number) => {
                        const isWorkerCell = cellIndex === 0;
                        const isStoryPointCell =
                          cellIndex > 0 && cellIndex < tableData.headers.length - 1;
                        const isClickableCell =
                          (isWorkerCell ||
                            (isStoryPointCell && typeof cell === 'number' && cell > 0)) &&
                          !isTotalRow;
                        const isWorkerSelected = isWorkerCell && selectedWorker === cell;
                        const isCellSelected =
                          isStoryPointCell &&
                          selectedSprint === tableData.headers[cellIndex] &&
                          selectedWorker === row[0];

                        const bgStyle =
                          isWorkerSelected || isCellSelected
                            ? {
