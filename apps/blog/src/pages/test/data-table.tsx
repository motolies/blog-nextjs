...
export default function DataTableTestPage() {
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ value }: { value: string }) => (
          <Badge
            variant={value === '관리자' ? 'default' : value === '편집자' ? 'secondary' : 'outline'}
          >
            {value}
          </Badge>
        ),
  ...
  const handleCellEdit = useCallback(
    ({ rowId, columnId, value }: { rowId: string; columnId: string; value: any }) => {
      setData((prev) =>
  ...
  const handleDeleteSelected = useCallback((selectedRows: any[], clearSelection: () => void) => {
  ...
  const getRowId = useCallback((row: any) => String(row.id), []);

  const isRowModified = useCallback((row: any) => modifiedIds.has(row.id), [modifiedIds]);

  const getRowClassName = useCallback(
    ({ row }: { row: any }) => (modifiedIds.has(row.id) ? 'bg-amber-50/60' : ''),
    [modifiedIds],
  );
      ...
      <div className="admin-workspace">
        <div className="admin-panel admin-panel-pad mb-4">
          ...
          <ul className="text-xs text-[color:var(--admin-text-muted)] space-y-1">
            <li>
              1. <strong>칼럼 이동</strong> — 헤더를 드래그하여 칼럼 순서 변경 (5px 이상 이동)
            </li>
            <li>
            ...
            <li>
              4. <strong>행 선택</strong> — 체크박스로 개별/전체 선택, 선택 시 툴바에 N개 선택됨
              표시
            </li>
            <li>
            ...
            <li>
              6. <strong>행 추가</strong> — 툴바 &quot;추가&quot; 버튼으로 새 행 생성 (노란 배경
              표시)
            </li>
            <li>
              7. <strong>전체 저장</strong> — 툴바 &quot;저장&quot; 버튼 클릭 시 토스트 메시지 확인
            </li>
            <li>
