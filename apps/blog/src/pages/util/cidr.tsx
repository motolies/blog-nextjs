...
export default function CidrPage() {
  ...
  const rangeResult = useMemo(
    () => rangeToCidrBlocks(rangeStart, rangeEnd),
    [rangeStart, rangeEnd],
  );
  const splitSource = useMemo(() => parseCidr(splitCidr), [splitCidr]);
  const splitResult = useMemo(
    () => splitSubnet(splitCidr, Number(splitPrefix), SPLIT_LIMIT),
    [splitCidr, splitPrefix],
  );
  ...
  const handlePrefixPick = useCallback((prefix) => {
    ...
    setTabValue('cidr');
    showToast(`/${prefix} 프리픽스를 적용했습니다.`, 'info');
  }, []);

  const renderValueCard = (label, value, copyLabel) => (
    <div className="border rounded-md p-3">
      <div className="flex items-start justify-between gap-2">
        <p className="text-xs text-dl-fg-muted">{label}</p>
        <button
          type="button"
          onClick={() => void handleCopy(value, copyLabel || label)}
          className="p-0.5 rounded hover:bg-dl-option-hover"
          title="복사"
        >
      ...
      <div className="border rounded-md p-4 mb-4">
        ...
        <div className="relative">
          <Input
            ...
            className={`pr-8 font-mono ${cidrResult.error ? 'border-dl-error' : ''}`}
          />
          <button
            type="button"
        ...
        {cidrResult.error ? (
          <p className="text-xs text-dl-error mt-1">{cidrResult.error}</p>
        ) : (
          <p className="text-xs text-dl-fg-muted mt-1">
            네트워크 주소가 아닌 값(10.0.0.5/8)을 입력해도 네트워크 주소로 정규화됩니다.
          </p>
        )}

        <div className="mt-3">
          <p className="text-xs text-dl-fg-muted mb-2">자주 쓰는 대역:</p>
          <div className="flex flex-wrap gap-1">
            {CIDR_PRESETS.map((preset) => (
              <button
                ...
                className="text-xs px-2 py-0.5 border rounded-full font-mono hover:bg-dl-option-hover transition-colors"
              >
                {preset.cidr}
              </button>
            ))}
      ...
      {cidrResult.info && (
        <>
          <div className="border rounded-md p-4 mb-4 bg-dl-info-bg">
            <div className="flex items-start justify-between gap-2">
              <p className="text-xs text-dl-fg-muted mb-1">IP 주소 범위</p>
              <button
                type="button"
                onClick={() =>
                  void handleCopy(
            ...
            <div className="flex flex-wrap items-center gap-1.5 mt-2">
              <span className="rounded-full border px-2 py-0.5 text-xs font-mono">
                {cidrResult.info.cidr}
              </span>
              <span className="rounded-full border px-2 py-0.5 text-xs">
                클래스 {cidrResult.info.networkClass}
              </span>
              <Badge tone={cidrResult.info.isPrivate ? 'warning' : 'success'} size="sm">
                {cidrResult.info.isPrivate ? '사설/특수 대역' : '공인 IP 대역'}
              </Badge>
              <span className="rounded-full bg-dl-option-hover px-2 py-0.5 text-xs text-dl-fg-muted">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 mb-4">
            ...
            {renderValueCard(
              '브로드캐스트 주소',
              cidrResult.info.broadcastAddress,
              '브로드캐스트 주소가',
            )}
            ...
            {renderValueCard(
              '와일드카드 마스크',
              cidrResult.info.wildcardMask,
              '와일드카드 마스크가',
            )}
            {renderValueCard(
              '전체 주소 수',
              formatCount(cidrResult.info.totalAddresses),
              '전체 주소 수가',
            )}
            {renderValueCard(
              '사용 가능 호스트',
              formatCount(cidrResult.info.usableHosts),
              '사용 가능 호스트 수가',
            )}
          </div>
          ...
          <div className="border rounded-md p-4 mb-4">
            <p className="font-medium mb-3">
              <span className="flex items-center gap-2">
                <ScanSearch className="h-4 w-4 text-dl-primary-ink" />
                IP 포함 여부 검사
              </span>
            </p>
            <Input
              ...
              className={`font-mono ${containsResult?.error ? 'border-dl-error' : ''}`}
            />
            {containsResult?.error && (
              <p className="text-xs text-dl-error mt-1">{containsResult.error}</p>
            )}
            {containsResult && !containsResult.error && containsResult.contained !== null && (
              <div className="mt-3 flex flex-wrap items-center gap-2 text-sm">
                <span
                  className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${
                    containsResult.contained
                      ? 'bg-dl-success-bg text-dl-success-ink'
                      : 'bg-dl-locked-bg text-dl-locked-ink'
                  }`}
                >
    ...
    <>
      <div className="border rounded-md p-4 mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <p className="text-xs text-dl-fg-muted mb-1">시작 IP</p>
            <Input
              value={rangeStart}
          ...
          <div>
            <p className="text-xs text-dl-fg-muted mb-1">끝 IP</p>
            <Input
              value={rangeEnd}
        ...
        {rangeResult.error ? (
          <p className="text-xs text-dl-error mt-2">{rangeResult.error}</p>
        ) : (
          <p className="text-xs text-dl-fg-muted mt-2">
            임의의 범위는 하나의 CIDR로 표현되지 않으므로 여러 블록으로 분해됩니다.
          </p>
        )}
      </div>
        ...
        <div className="border rounded-md p-4 mb-4">
          <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
            <p className="font-medium">
              ...
              <span className="ml-2 text-xs font-normal text-dl-fg-muted">
            ...
            <Button
              ...
              onClick={() =>
                void handleCopy(
                  rangeResult.blocks.map((block) => block.cidr).join('\n'),
                  'CIDR 목록이',
                )
              }
            >
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  ...
                  <th className="text-left py-2 px-3 font-medium text-dl-fg-muted">CIDR</th>
                  <th className="text-left py-2 px-3 font-medium text-dl-fg-muted">범위</th>
                  <th className="text-left py-2 px-3 font-medium text-dl-fg-muted">넷마스크</th>
                  <th className="text-right py-2 px-3 font-medium text-dl-fg-muted">주소 수</th>
                </tr>
              </thead>
              <tbody>
                {rangeResult.blocks.map((block, index) => (
                  <tr key={block.cidr} className="border-b last:border-0 hover:bg-dl-option-hover">
    ...
    <>
      <div className="border rounded-md p-4 mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-[1fr_10rem] gap-3">
          <div>
            <p className="text-xs text-dl-fg-muted mb-1">원본 CIDR</p>
            <Input
              value={splitCidr}
              onChange={(e) => setSplitCidr(e.target.value)}
              placeholder="192.168.0.0/24"
              className={`font-mono ${splitSource.error ? 'border-dl-error' : ''}`}
            />
          </div>
          <div>
            <p className="text-xs text-dl-fg-muted mb-1">분할 프리픽스</p>
            <Select
              value={splitPrefix}
              onValueChange={setSplitPrefix}
              placeholder="/26"
              options={splitPrefixOptions.map((row) => ({
                value: String(row.prefix),
                label: `/${row.prefix} (${formatCount(row.usableHosts)} 호스트)`,
              }))}
        ...
        {splitResult.error ? (
          <p className="text-xs text-dl-error mt-2">{splitResult.error}</p>
        ) : (
          <p className="text-xs text-dl-fg-muted mt-2">
            원본 대역을 동일한 크기의 서브넷으로 나눕니다.
          </p>
        )}
      </div>
      ...
      {splitResult.subnets.length > 0 && (
        <div className="border rounded-md p-4 mb-4">
          ...
          {splitResult.truncated && (
            <p className="mb-3 rounded-md border border-dl-warning bg-dl-warning-bg px-3 py-2 text-xs text-dl-warning-ink">
              전체 {formatCount(splitResult.totalCount)}개 중 처음{' '}
              {formatCount(splitResult.subnets.length)}개만 표시합니다.
            </p>
          )}
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  <th className="text-left py-2 px-3 w-12 font-medium text-dl-fg-muted">#</th>
                  <th className="text-left py-2 px-3 font-medium text-dl-fg-muted">네트워크</th>
                  <th className="text-left py-2 px-3 font-medium text-dl-fg-muted">
                    사용 가능 범위
                  </th>
                  <th className="text-left py-2 px-3 font-medium text-dl-fg-muted">브로드캐스트</th>
                  <th className="text-right py-2 px-3 font-medium text-dl-fg-muted">호스트</th>
                </tr>
              </thead>
              <tbody>
                {splitResult.subnets.map((subnet, index) => (
                  <tr key={subnet.cidr} className="border-b last:border-0 hover:bg-dl-option-hover">
                    <td className="py-2 px-3 text-dl-fg-muted">{index + 1}</td>
                    <td className="py-2 px-3 font-mono">
                      <button
                        type="button"
  ...
  const renderGuide = () => (
    <Accordion type="single" collapsible>
      <AccordionItem
        value="guide"
        className="mt-4 overflow-hidden rounded-[1.25rem] border border-dl-border bg-dl-surface shadow-dl-card"
      >
          ...
          <div className="flex min-w-0 items-start gap-3">
            ...
            <span className="min-w-0">
              ...
              <span className="mt-1 block text-sm leading-6 text-dl-fg-muted">
                /0부터 /32까지 넷마스크와 주소 수를 한 번에 확인하고, 행을 눌러 바로 적용할 수
                있습니다.
              </span>
            </span>
          </div>
        ...
        <AccordionContent>
          <div className="border-t border-dl-divider bg-dl-surface px-4 pt-4 pb-4">
            <div className="max-h-96 overflow-auto">
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-dl-grid-header">
                  <tr className="border-b">
                    <th className="text-left py-2 px-3 w-20 font-medium text-dl-fg-muted">
                      프리픽스
                    </th>
                    <th className="text-left py-2 px-3 font-medium text-dl-fg-muted">넷마스크</th>
                    <th className="text-left py-2 px-3 font-medium text-dl-fg-muted">와일드카드</th>
                    <th className="text-right py-2 px-3 font-medium text-dl-fg-muted">주소 수</th>
                    <th className="text-right py-2 px-3 font-medium text-dl-fg-muted">호스트 수</th>
                  </tr>
                </thead>
                <tbody>
                  {CIDR_TABLE.map((row) => (
  ...
  return (
    <div className="p-2 sm:p-4">
      <div className="flex items-center gap-2 mb-4">
        ...
        <Network className="h-5 w-5 text-dl-primary-ink" />
      </div>
      ...
      <div className="border rounded-md">
        <Tabs value={tabValue} onValueChange={setTabValue}>
          <TabList className="w-full grid grid-cols-3 rounded-none border-b">
            ...
            <Tab value="split">서브넷 분할</Tab>
          </TabList>

          <div className="p-2 sm:p-4">
            <TabPanel value="cidr">{renderCidrTab()}</TabPanel>
            <TabPanel value="range">{renderRangeTab()}</TabPanel>
            <TabPanel value="split">{renderSplitTab()}</TabPanel>
          </div>
        </Tabs>
      </div>
      ...
      <div className="mt-4 p-3 bg-dl-option-hover rounded-md">
        ...
        <p className="text-sm text-dl-fg-muted">
          CIDR(Classless Inter-Domain Routing)은 <span className="font-mono">192.168.0.0/24</span>
          처럼 네트워크 주소와 프리픽스 길이로 IP 대역을 표기하는 방식입니다. 프리픽스는 앞쪽 몇
          비트가 네트워크를 가리키는지를 의미하며, 남은 비트 수만큼 주소를 담을 수 있습니다(
          <span className="font-mono">/24</span> → 256개). 일반적으로 네트워크 주소와 브로드캐스트
          주소는 호스트에 할당하지 않으므로 사용 가능 호스트는 2개 적지만,
          <span className="font-mono"> /31</span>은 라우터 간 P2P 링크용으로 2개(RFC 3021),
          <span className="font-mono"> /32</span>는 단일 호스트로 취급합니다. 이 도구는 IPv4만
          지원하며 모든 계산은 브라우저에서 처리됩니다.
        </p>
      </div>
    </div>
  );
