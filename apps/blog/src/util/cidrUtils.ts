...
export const isValidIpv4 = (ip: string): boolean => {
  ...
  return octets.every(
    (octet) =>
      /^\d{1,3}$/.test(octet) && Number(octet) <= 255 && (octet === '0' || !octet.startsWith('0')),
  );
};
...
export const netmaskLongFromPrefix = (prefix: number): number =>
  IPV4_ADDRESS_SPACE - 2 ** (32 - prefix);
...
export const rangeToCidrBlocks = (
  startIp: string,
  endIp: string,
): { blocks: CidrInfo[]; error: string | null } => {
  if (!isValidIpv4(startIp)) {
    return {
      blocks: [],
      error: `시작 IP 형식이 올바르지 않습니다: ${(startIp || '').trim() || '(비어 있음)'}`,
    };
  }
  ...
  if (!isValidIpv4(endIp)) {
    return {
      blocks: [],
      error: `끝 IP 형식이 올바르지 않습니다: ${(endIp || '').trim() || '(비어 있음)'}`,
    };
  }
...
export const splitSubnet = (
  ...
  if (!Number.isInteger(newPrefix) || newPrefix < 0 || newPrefix > 32) {
    return {
      ...
      totalCount: 0,
      truncated: false,
      error: '분할 프리픽스는 0~32 사이의 숫자여야 합니다.',
    };
  }
...
export const isIpInCidr = (
  ip: string,
  cidr: string,
): { contained: boolean | null; info: CidrInfo | null; error: string | null } => {
  if (!isValidIpv4(ip)) {
    return {
      contained: null,
      info: null,
      error: `IPv4 주소 형식이 올바르지 않습니다: ${(ip || '').trim() || '(비어 있음)'}`,
    };
  }
