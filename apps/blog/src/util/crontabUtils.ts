...
export const DAY_NAMES: string[] = ['일', '월', '화', '수', '목', '금', '토']
export const MONTH_NAMES: string[] = ['1월', '2월', '3월', '4월', '5월', '6월', '7월', '8월', '9월', '10월', '11월', '12월']

export const generateKoreanDescription = (expression: string, isSpring: boolean = false): string => {
    try {
...
export const calculateNextRuns = (expression: string, runCount: number = 10, timezone: string = 'Asia/Seoul'): { runs: string[]; error: string | null } => {
    try {
        ...
        return {
            runs: [],
            error: (e as Error).message || '유효하지 않은 cron 표현식입니다.'
        }
    }
}
