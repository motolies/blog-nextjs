...
export const generateKoreanDescription = (
  expression: string,
  ...
  try {
    ...
    if (month !== '*' && month !== '?') {
      if (month.includes(',')) {
        descriptions.push(
          month
            .split(',')
            .map((value) => MONTH_NAMES[parseInt(value, 10) - 1] || value)
            .join(', '),
        );
      } else if (month.includes('-')) {
        const [start, end] = month.split('-');
        descriptions.push(
          `${MONTH_NAMES[parseInt(start, 10) - 1]}부터 ${MONTH_NAMES[parseInt(end, 10) - 1]}까지`,
        );
      } else {
    ...
    if (dayOfWeek !== '*' && dayOfWeek !== '?') {
      if (dayOfWeek.includes('#')) {
        ...
        const ordinal =
          ['첫째', '둘째', '셋째', '넷째', '다섯째'][parseInt(nth, 10) - 1] || `${nth}번째`;
        ...
        descriptions.push(
          `${dayOfWeek
            .split(',')
            .map((value) => DAY_NAMES[parseInt(value, 10)] || value)
            .join(', ')}요일`,
        );
      } else if (dayOfWeek.includes('-')) {
