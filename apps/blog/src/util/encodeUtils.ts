...
export const encodeUnicodeEscapes = (value: string): string => (
    Array.from(value)
        .flatMap((char) => {
...
export const formatJsonText = (value: string): string => JSON.stringify(parseJsonText(value), null, 2)

export const minifyJsonText = (value: string): string => JSON.stringify(parseJsonText(value))
...
export const decodeJwtToken = (token: string): { header: unknown; payload: unknown; signature: string } => {
