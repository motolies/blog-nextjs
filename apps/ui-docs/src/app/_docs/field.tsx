...
export const fieldDoc: DocEntry = {
  ...
  title: 'Field',
  description:
    '폼 한 칸의 래퍼 — 라벨·필수 표시·가이드(help)·오류(error)를 자식 컨트롤의 aria-* 로 자동 배선한다. error 가 있으면 자식이 오류 배색 + aria-invalid 를 입고 help 는 감춰진다. 필수값 오류를 모달로 막지 않는다(v3 §ds-05) — 못 채운 칸 전부에 동시에 표시한다.',
  usage: USAGE,
  examples: [
    {
      id: 'states',
  ...
  propsTables: [
    {
      ...
      rows: definePropRows<FieldProps>()([
        ...
        },
        {
          ...
          defaultValue: "'edit'",
          description:
            '폼 모드 — 생략하면 감싼 FormMode 의 값. view 는 값 텍스트(폼 값이 안 나와 전환 폼은 제어형 필수), disabled 는 컨트롤 유지 + 비활성(FormData 제외). 명시 prop 이 컨텍스트를 이긴다.',
        },
        {
          name: 'view',
          type: 'ReactNode',
