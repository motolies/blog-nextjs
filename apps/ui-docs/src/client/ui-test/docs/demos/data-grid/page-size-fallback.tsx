 ...
 *
 * 확인: 레거시 → pageSize undefined, 게시글 ID 폭 180 **유지** / 25 → ui 는 25 를 그대로 주지만 셀렉트는 10 /
...
const FIXTURES: readonly Fixture[] = [
  {
    ...
    raw: '{"version":1,"widths":{"postId":180},"hidden":[],"order":[]}',
    expectation: 'pageSize undefined → 셀렉트 10 · 게시글 ID 폭 180 유지',
  },
  {
    id: 'out-of-list',
    label: '목록 밖 값 25',
    raw: '{"version":1,"widths":{"postId":180},"hidden":[],"order":[],"pageSize":25}',
    expectation: 'ui 는 25 를 그대로 준다 → 셀렉트는 10(목록은 앱이 안다) · 폭 180 유지',
  },
  {
    id: 'zero',
    label: '불량 값 0',
    raw: '{"version":1,"widths":{"postId":180},"hidden":[],"order":[],"pageSize":0}',
    expectation: 'pageSize 항목만 탈락(undefined) · 폭 180 유지',
  },
  {
    id: 'string',
    label: "불량 값 '20' (문자열)",
    raw: '{"version":1,"widths":{"postId":180},"hidden":[],"order":[],"pageSize":"20"}',
    expectation: '숫자가 아니면 항목 단위 탈락(undefined) · 폭 180 유지',
  },
  {
    id: 'version',
    label: 'version 2',
    raw: '{"version":2,"widths":{"postId":180},"hidden":[],"order":[],"pageSize":20}',
...
function FallbackBlock() {
  ...
  const postIdWidth = preference.widths.postId;
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="grid grid-cols-2 gap-2 lg:grid-cols-4">
        ...
        <Stat
          label="preference.widths.postId"
          value={postIdWidth === undefined ? 'undefined — 기본 140' : String(postIdWidth)}
        />
      </div>
      ...
      <div className="flex flex-col">
        <DataGrid
          columns={COLUMNS}
          rows={DEMO_POSTS.slice(0, pageSize)}
