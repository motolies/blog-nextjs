 ...
 *   · `FieldValue` — 애초에 **고칠 대상이 아닌** 값(주문번호·등록일시). 모드와 무관하게 항상 표시다
 ...
 * · 표시값 ≠ 편집값인 주문상태는 `Field` 의 `view` prop 이 Badge 로 덮는다
...
export function FormGridDetailModesDemo() {
  ...
  const [orderDate, setOrderDate] = useState('2026-07-15');
  const [receiver, setReceiver] = useState('김민준');
        ...
        <div className="flex flex-col gap-dl-gutter rounded-dl-container bg-dl-canvas p-dl-gutter">
          <Card>
            ...
            <FormGrid>
              ...
              <Field label="서비스타입" htmlFor="dm-service" required>
                <Select
                  id="dm-service"
                  placeholder="선택"
              ...
              <Field label="주문일" htmlFor="dm-orderDate" required>
                <DatePicker id="dm-orderDate" value={orderDate} onValueChange={setOrderDate} />
              </Field>
          ...
          <Card>
            <CardHeader title="수취인 정보" />
            <FormGrid>
              <Field label="수취인" htmlFor="dm-receiver" required>
                <Input
                  id="dm-receiver"
                  value={receiver}
              ...
              <Field label="배송지" htmlFor="dm-address" className="col-span-full">
                <Input
                  id="dm-address"
                  value={address}
              ...
              <Field label="배송 요청사항" htmlFor="dm-memo" className="col-span-full">
                <Textarea
                  id="dm-memo"
                  value={memo}
