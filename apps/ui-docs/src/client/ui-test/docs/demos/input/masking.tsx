 ...
 * · **FormData 덤프에 email·phone 키 자체가 없다** — masking 은 `name` 을 전달하지 않아
 *   마스킹된 값(a***@b.com)이 저장 요청에 실려 실값을 파괴하는 사고를 구조적으로 막는다
 *   (현행에 실제 사고 사례가 있다 — 서버 zod `.omit()` 방어선과 이중 방어).
 *   서버 계약이 "키 없음 = 변경 없음"(zod partial)일 때만 안전한 이유이기도 하다
 * · 일반 칸(receiver)은 그대로 실린다 — 마스킹 칸만 선별적으로 빠진다
...
export function InputMaskingDemo() {
  ...
  return (
    <form onSubmit={showFormData} className="flex w-full max-w-xl flex-col gap-4">
      <FormGrid>
        <Field label="수취인" htmlFor="mk-receiver">
          <Input id="mk-receiver" name="receiver" defaultValue="김민준" />
        </Field>
        ...
        <Field label="연락처" htmlFor="mk-phone" help="마스킹 — 앞 3자리 외 ********">
          <Input id="mk-phone" name="phone" masking value="010********" />
        </Field>
      </FormGrid>
      ...
      {dump === '' ? null : (
        // 폼이 실제로 실어 보내는 값 — receiver 만 있고 email·phone 키는 존재하지 않아야 한다.
