...
export default function DataTableTestPage() {
  ...
  const handleSaveAll = useCallback((allData: any[]) => {
  ...
  const handleCellEdit = useCallback(({ rowId, columnId, value }: { rowId: string; columnId: string; value: any }) => {
  ...
  const isRowModified = useCallback(
    (row: any) => modifiedIds.has(row.id),
    [modifiedIds],
  )
